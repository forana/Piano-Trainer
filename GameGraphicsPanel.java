...
public class GameGraphicsPanel extends JPanel implements NoteEventListener, ProcessedNoteEventListener, MidiEventListener, Updatable{
	...
	private HashMap<Note,Long> fallingNotes;

	/** A Map of the "Correct!" and "Miss!" response messages **/
	private HashMap<ProcessedNoteEvent,Integer> fallingResponse;

	/** A List of the currently held piano keys **/
	private ArrayList<Integer> midiNotesPressed;
		...
		synchronized (GameGraphicsPanel.class) 
		{
			...
			for(Integer i: midiNotesPressed){
				if(isSharp(i)){
					...
					g.setColor(new Color((int) (170-(40.0*(1.0/((((int)System.currentTimeMillis())-timeOfNote)/500.0+1.0)))),200,255));
		...
		synchronized (GameGraphicsPanel.class) {
			for(Note n: fallingNotes.keySet()){
				...
				if(isSharp(n.getPitch())){
					...
					color+=70;
					g.setColor(new Color((int)color,(int)color,(int)color));
					g.drawRect(nX(findXForKey(n.getPitch())-(1004/numWhiteKeys/4)), nY(y-duration), nX((1004/numWhiteKeys/2)), nY(duration));
				}
				//if its a white key
				else{
			...
			//Display the title of the song in the upper left corner
			g.setColor(Color.black);
	...
	public void handleNoteEvent(NoteEvent e) {
		if(e.getAction()==NoteAction.BEGIN){
			synchronized (GameGraphicsPanel.class) {
				//if the note event is for one of the active tracks, make it fall
	...
	public void handleProcessedNoteEvent(ProcessedNoteEvent e) {
		//a note has been processed, add it to the "Correct! / Miss!" display list
	...
	public void handleMidiEvent(MidiEvent midiEvent) {
		//when a key is pressed, add it to the light-up-lane list
		//when released, remove it
		synchronized (GameGraphicsPanel.class) {
	 ...
	 * stop
	 * 
	 * called to halt the song engine pieces
	 * 
	 */
	public void stop()
	{
	 ...
	 * play
	 * 
	 * starts the song engine and begins repainting the screen (plays the song)
	 */
	public void play(){
		//Start repainting
