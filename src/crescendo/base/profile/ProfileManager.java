 ...
 * ProfileManager
 * 
 * This singleton class represents the collection of profiles that have been
 * created by the application. It holds a profile as the active profile and a
 * collection of profiles for the rest. Since the individual preferences exist
 * within the profile class, this essentially holds all of the information that
 * needs to be persistent between runs, and therefore is serializable and has
...
public class ProfileManager implements Serializable {
	 ...
	 * 
	 * **NOTE** THIS WILL NOT SAVE PREVIOUS ACTIVE PROFILE
	 * 
	 * @param p
	 *            - profile to set to activeProfile
	 */
	 ...
	 * 
	 * @param p
	 *            - the profile to switch to activeProfile
	 * @return
	 */
	...
	public Profile getProfileByName(String name) {
		...
		for (Profile p : profiles)
			if (p.getName().equals(name))
				toRet = p;
		if (activeProfile.getName().equals(name))
			toRet = activeProfile;
		return toRet;
	}
	...
	public boolean loadFromFile(String filename) {
		...
		try {
			...
			profiles.clear();
			for (Profile p : loaded.getProfiles())
				profiles.add(p);
		} catch (IOException ex) {
			// ex.printStackTrace();
			toRet = false;
